...
export function CreateRFQModal({ isOpen, onClose }: CreateRFQModalProps) {
  ...
  const createRfqMutation = useMutation(
    trpc.createPropertyManagerRFQ.mutationOptions({
      onSuccess: () => {
        const successMessage = selectedContractors.length > 0 
          ? `RFQ submitted successfully! Sent to ${selectedContractors.length} contractor(s).`
          : "RFQ submitted successfully! Awaiting quotation.";
  ...
  const onSubmit: SubmitHandler<RFQFormInput> = (data) => {
    if (!token) {
      toast.error("Authentication required to submit RFQ.");
      return;
    }
  ...
  const handleBuildingSelect = (buildingId: string) => {
    ...
    if (buildingId) {
      ...
      if (building) {
        setValue("buildingAddress", building.address);
        setValue("buildingName", building.name);
      }
    }
  };
  ...
  const isSubmitting = createRfqMutation.isPending;
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">
                            Select Contractors to Receive RFQ (Optional)
                          </label>
                          {contractorsQuery.isLoading ? (
                            ...
                            <div className="max-h-48 overflow-y-auto border border-gray-300 rounded-lg p-3 space-y-2">
                              {contractors.map((contractor: any) => {
                                // Show all contractors - backend will handle portal access check
                                return (
                                  <label
                                    key={contractor.id}
