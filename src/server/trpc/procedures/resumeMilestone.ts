...
export const resumeMilestone = baseProcedure
  .input(
    z.object({
      token: z.string(),
  ...
  .mutation(async ({ input }) => {
    try {
      ...
      // Check if milestone is on hold
      if (milestone.status !== "ON_HOLD") {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Milestone must be on hold to resume",
        });
      }

      // Update milestone back to in progress
